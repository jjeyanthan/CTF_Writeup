...
def phase_c(p, n=0):  # permet de trouver le resultat attendu que j'appelle "knwo_final_values"
    if p ==[]:
        return n!=0
    if len(p) >= 2:
        f= p[0]
        rest = p[1::]
        rd.seed(s[n])
        return (rd.randint(0,30) == f) and phase_c(rest, n+1)


def simpleBruteForce(good_val):
    
    flag=""
    
    max_counter=0
    for i in range(len(good_val)//10):
        for char in range(128):
            testChar=flag + chr(char)
            testChar+= (len(good_val)//10 - len(testChar)) * '*'
           
            result = b(testChar,1)
            counter=0
            for j in range(len(good_val)):
                if result[j] == good_val[j]:
                    counter+=1
            
            print("COUNTER: " ,  counter , " TEST CARACT ", testChar)


            if max_counter < counter:
                max_counter = counter
                choose_letter= chr(char)
...
simpleBruteForce(knwo_final_values)